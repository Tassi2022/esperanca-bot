...
CONCORRENTES = CONCORRENTES_POR_PIZZARIA.get("esperanca_saude", [])
...
def salvar_concorrente(dados, pizzaria_id="esperanca_saude"):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT INTO concorrentes (username,nome,seguidores,seguindo,posts,coletado_em,pizzaria_id,curtidas,media_curtidas) VALUES (?,?,?,?,?,?,?,?,?)",
        (dados["username"], dados["nome"], dados["seguidores"],
         dados["seguindo"], dados["posts"], datetime.now().strftime("%d/%m %H:%M"), pizzaria_id,
         dados.get("curtidas", 0), dados.get("media_curtidas", 0)))
    conn.commit()
    conn.close()
...
def coletar_concorrentes(pizzaria_id=None):
    ...
    pid = pizzaria_id or "esperanca_saude"
    for c in lista:
        ...
        if dados:
            ...
            total_posts, total_curtidas = buscar_posts_e_curtidas_24meses(c["username"])
            dados["curtidas"] = total_curtidas
            dados["media_curtidas"] = round(total_curtidas / total_posts, 2) if total_posts else 0
            salvar_concorrente(dados, pid)
            salvar_curtidas({"username": c["username"], "nome": c["nome"], "total_curtidas": total_curtidas})
...
def salvar_historico(nome, username, historico):
    if not historico:
        logger.warning(f"Historico vazio para {username}, mantendo dados existentes")
        return
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def buscar_posts_apify(username, limite_posts=90):
    ...
    try:
        ...
        logger.error(f"Erro Apify {username}: {e}")
        return []

    if not isinstance(data, list):
        return []

    limite = datetime.now(timezone.utc) - timedelta(days=730)
    posts = []
    for item in data:
        ...
        posts.append({
            ...
            "link": item.get("url", ""),
            "curtidas": item.get("likesCount", 0) or 0,
            "comentarios": item.get("commentsCount", 0) or 0,
            "texto": (item.get("caption") or "")[:150],
            "taken_at_iso": ts,
        })
    return sorted(posts, key=lambda x: x["curtidas"], reverse=True)


def buscar_historico_apify(username, limite_posts=90):
    """Constroi historico mensal (curtidas+posts por mes) via Apify, ultimos 24 meses."""
    posts = buscar_posts_apify(username, limite_posts=limite_posts)
    historico = {}
    for p in posts:
        ts = p.get("taken_at_iso")
        if not ts:
            continue
        from datetime import datetime
        dt = datetime.fromisoformat(ts.replace("Z", "+00:00"))
        ...
        if chave not in historico:
            historico[chave] = {"curtidas": 0, "posts": 0}
        historico[chave]["curtidas"] += p["curtidas"]
        historico[chave]["posts"] += 1
    return historico


def buscar_perfil_apify(username):
    """Busca dados de perfil (seguidores, posts, nome) via Apify Instagram Scraper."""
    url = f"https://www.instagram.com/{username}/"
    run_input = {
        "directUrls": [url],
        "resultsType": "details",
    ...
    try:
        ...
        logger.error(f"Erro Apify perfil {username}: {e}")
        return None
    ...
    return {
        ...
        "seguindo": item.get("followsCount", 0) or 0,
        "posts": item.get("postsCount", 0) or 0,
        "nome": item.get("fullName", username),
    }
